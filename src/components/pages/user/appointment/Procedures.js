...
import { getProcList, getProcTypes } from '../../../../api/Catalog.js';
...
export default function Procedures() {
	...
	return (
		<div>
			<h1 className="font-bold text-2xl mb-4">Запись на процедуры</h1>
			<div className="grid grid-cols-4 gap-4">
				<div className="col-span-3 flex flex-col gap-3">
					<select value={filters.type} onChange={handleChange} className="border border-pistachio-dark rounded-lg p-2 bg-white outline-none w-64 text-xs font-semibold">
						{procTypes.map((type) => <option key={type} value={type}>{type}</option>)}
					</select>
					<h2 className="text-gray-500 text-xs font-medium">Сортировка по {filters.sort}</h2>
					<List filters={filters} Card={ProcCard} getList={getProcList}/>
				</div>
